...
fn tool_defs(
    ...
    if matches!(role, Role::Orchestrator | Role::Worker | Role::Planner) {
        tools.push(tool(
            "post_issue_comment",
            "Post a comment on a GitHub issue in this group's repo, and get the new comment's URL back. THIS IS HOW A PLANNER DELIVERS ITS PLAN: write the plan, post it here, then report — do not shell out to `gh issue comment`, which cannot carry a document (a Bash command over 10,000 characters, or one containing newlines, matches no permission rule and is denied outright in a planner's pane). `body` is full GitHub-flavoured markdown, passed as data — a leading `-`, quotes, backticks, fenced code and newlines all survive verbatim, so paste the comment exactly as you want it to read. There is no length limit here beyond the MCP request itself. The repo is resolved from YOUR OWN group; you cannot name one. COMMENTS ONLY: this tool cannot label, close, reopen, merge, review, or create anything — those are not arguments it refuses, they are verbs it does not have. Note that GitHub numbers issues and pull requests in one namespace, so an `issue` that is really a PR number posts to that PR's conversation; it still cannot do anything to it but comment. An empty or whitespace-only body is rejected before anything is posted. ONE CASE WHERE THIS TOOL VALIDATES YOUR BODY, and it is not a regression: if you are a PLANNER spawned by a plan drive, and that drive is still waiting for its plan, the `orrerix-plan` block in your comment is checked BEFORE anything is posted — a missing or invalid block comes back as an error listing the reasons with their line numbers, and NOTHING reaches the issue. Fix the block and call this again; you are still in your own turn, so it costs no round trip to anyone. Three refusals park the drive with the last reasons. Every other caller, and every later comment once the drive has its plan, posts unvalidated exactly as before. Every post that reaches `gh` is audited as an `issue-comment` row carrying the issue, the body size and the outcome — the URL when it worked, the error when it did not — so the human can see what you published, and what you failed to publish, without reading your pane. A call that is refused before `gh` runs (an empty body, or orrerix being unable to write the body file at all) writes no row.",
            json!({
        ...
        tools.extend([
            tool("spawn_agent",
                ...
                json!({
                    ...
                    "base": { "type": "string", "description": "Start-point for the worktree branch (default: the repo's default branch, fetched fresh from origin). Pass a feature branch (e.g. 'feat/x' or 'origin/feat/x') to deliberately stack this worktree on top of it. Ignored without worktree=true. When 'branch' already exists, that branch is checked out as-is (its history stands on its own) — a branch that exists only on origin (e.g. an open PR whose earlier worktree was pruned) is checked out tracking origin/<branch>, never re-cut from base (#3405) — but if it does NOT descend from the requested base, the spawn fails loudly (#227) rather than silently handing back a wrong-base worktree." },
            ...
            // contract break.
            tool("list_deferred", "Read what delivery triage has HELD BACK from your pane: `{enabled, provider, max_defer_minutes, count, items:[{ts_ms, from, kind, rule, text}]}`. When this repo declares `triage: enabled: true`, orrerix answers a notice whose LEADING SHAPE closes it without any judgement — a notify_when run or PR-checks watch that came back green AND whose registered note named nothing to do when it did, a planner that posted its plan and exited, a pane that exited having printed something, a cancelled drive, a mid-plan `---BEGIN PLAN k/n---` chunk — by recording it instead of waking you. Everything else is DELIVERED, which is the default for any shape the rules do not positively recognise, and a delivery that names you — the full marker set is `blocking on you`, `is yours`, `needs you`, `needs your`, `you must rule`, `your call`, matched anywhere in the text and case-insensitively — a HELD drive, a `blocked` report, a watchdog stall and a re-grounding notice are never triaged at all. A review drive reporting GATE SATISFIED is NEVER held: its notice ends `Disposition is yours (INVARIANT 3)`, which names you, and the rule that used to hold one was retired outright (#3324) — so write the green-path move into a watch's `note` when you register it, because a note saying what happens when it goes green is what keeps that verdict coming to you. NOTHING IS EVER DROPPED. Every held notice is in this list, on disk in this group's directory, and in the audit log as a `delivery-triaged` row CARRYING ITS FULL TEXT — that row is permanent, so a notice's words survive the flush that empties this list. It is flushed to you as ONE framed `[orrerix] N notices deferred…` line at the next delivery that DID need you, at `max_defer_minutes` if nothing does, or when the store fills. So you do not need to poll this: the flush comes to you. WHAT IT IS ACTUALLY FOR is the question the flush cannot answer — `what is being held RIGHT NOW`, in full text rather than quoted — when you are about to make a call that one of them might bear on, or when you are re-grounding after a compact and want to know what happened while you were not reading. `enabled: false` means this repo declares no triage at all and `count: 0` means nothing is held; read the flag before concluding the second. Read-only: calling this never delivers, flushes, or clears anything.",
                json!({}), &[]),
            ...
            tool("drive_review",
                "Hand ONE PR's worker-reviewer rounds to orrerix, so you stop spending a turn on each one. orrerix then does, on its own poll loop: wait for CI; spawn or resume each reviewer lane the merge gate requires, in gate order, briefing it with the head and what moved; hand a FAIL or a red run or a conflict back to the worker session you name here; and stop with ONE notice in this pane at gate-satisfied, at an escalate, or at a bound. A hold that repeats one it has already announced — the same LINE, word for word, which is what a resume that changed nothing produces — is on the audit log as `rd-hold-repeated` instead of in your pane. Anything the line says differently reaches you: a new head, a spent counter, a different refusal (a hand-back that failed another way, a cap refusal quoting a roster that has moved). TWO reasons are exempt even from an identical line: `state-stalled` and `drive-stalled` report how LONG the drive sat, rounded, so a repeat of the same sentence means it sat out the whole bound again. It never merges, never edits a PR or an issue, never writes a verdict, and never decides a disposition — those stay yours. It kills a pane in exactly two cases and no others (#2501): a reviewer lane whose verdict is recorded at the head it is driving, and a worker pane that has reported and gone idle once the drive has read that report — both released with the session kept, so the next round resumes the same conversation in a fresh pane, and both on the audit log as `rd-lane-released` / `rd-worker-released`. It never kills a pane to make room, and your own kill_agent authority is unchanged. While a drive is live, that PR's delegates' report and review_verdict notices go to the driver instead of appearing here; their message_orchestrator lines still reach you unchanged, and the drive then holds so you know why. Call it per PR, deliberately: it is NOT automatic on report(done) unless this repo's workflow sets driver.auto_drive_on_done (#3367) — and even then only for a worker's done naming its OWN open PR (its orrerix-recorded branch is the PR's head), never a [scratch] PR, never one already driven or already carrying a verdict; a refused one reaches you exactly as before, with its reason on the audit log as rd-auto-start-declined. The PRs where a drive is wrong are ordinary ones (a scratch or red-evidence PR, a release bump, a PR the human is reading themselves), which is why that key is off by default. Counters are INVARIANT 9's and clamp toward it, never away: three review rounds, three CI attempts, one rebase. ONE bounded exception (#2509): when a round's blocking fail comes back on a lane the driver had re-briefed about the PR BODY alone — the head unmoved since a full round was spent on it, every required lane already answered about that code — it hands back once more instead of parking at the bound. Once per drive, never for a fail about the code, audited `rd-round-grace`, and shown as `grace_used`; the next blocking fail after it parks the drive whatever it is about. Refuses `driver-disabled`, `pr-not-open`, `pr-unverifiable`, `resume-not-found`, `resume-ambiguous`, `resume-session-empty`, `already-driven`, `in-merge-queue` (that PR is in the merge queue, and the exclusion is mutual — queue_merge answers `in-review-drive` the other way round; the intended sequence is serial and has a direction: drive first, disposition the findings, THEN queue), `gate-not-configured`, `gate-names-no-such-block`, `worker-unresumable` (the session you named cannot take a hand-back: its block is this group's orchestrator or manager, or the roster no longer declares the block that session was minted under — the refusal's detail quotes which, and retrying or resuming cannot help; name a worker session instead). A hand-back that fails the same way twice (same session, same failure) says so — `second time:` in the hold's quoted refusal — because the remedy then is a different session or cancel_review_drive, not another resume. Four further reasons mean ORRERIX ITSELF FAILED rather than that the driver declined you: `rd-state-unreadable` (orrerix cannot read its own drive record, which is NOT 'nothing is driven'), `rd-state-unwritable` (the drive was computed and could not be saved, so it did not happen), `rd-unavailable`, and `gate-unreadable` (a gate file is present and orrerix cannot read it — NOT `gate-not-configured`, which means it is genuinely absent). Report one of those rather than working around it. Also resumes a HELD drive: call it again with the same PR, and pass reset_counters to spend a fresh budget.",
                json!({
            ...
            tool("drive_plan",
                ...
                json!({
                    ...
                    "review_minutes": { "type": "number", "description": "OPTIONAL, 0..=120, default from the repo's driver.plan_review_minutes. How long a posted plan waits before the drive acts on it, so you can veto. 0 means no window — the label already said go. Above zero the drive posts EXACTLY ONE notice and boards nothing until the window has run, so cancel_plan_drive during it stops the drive with nothing spawned." },
                    "base": { "type": "string", "description": "OPTIONAL. The branch slices are to be cut from, passed through to the planner's brief. Recorded, and used as the base every slice's worktree is cut from." },
                }),
                &["issue"]),
            tool("plan_drive_status", "Where this group's plan drives stand: {enabled, drives:[{issue, state, held_reason?, held_state?, consent, planner, planner_block, comment_url, invalid_count, last_invalid, review_minutes, slices?, since_ms, state_ms}]}. States are planning | plan-posted | plan-review | boarding | running | held; complete and cancelled are terminal and not listed. `held` is PARKED, not finished - it comes back with resume_plan_drive, or stops with cancel_plan_drive - and `held_reason` says which it is, while `held_state` says what the drive was doing when it parked. `slices` is the planner's own plan once one is stored - id, title, branch, block, deps, hold - plus what the drive has made of each: `task_id` (the board row), `slice_state` (queued | running | in-review | done | held), `slice_hold` (cap-full | worker-blocked | pr-closed | pr-missing | worker-gone), `agent` and `pr`. A SLICE hold parks that slice alone and the independent ones keep going; resume_plan_drive releases the slice holds with the drive. Terminal drives are not listed. Read this after a compaction: it is how you recover which issues orrerix is planning for you, and where each one got to. `refused: pd-state-unreadable` means orrerix cannot read its own record, which is NOT 'nothing is driven'. Read-only: calling this never changes anything.", json!({}), &[]),
...
fn call_tool(reg: &OrchRegistry, caller: &Caller, name: &str, args: &Value) -> Result<String, String> {
    ...
    match name {
        ...
        "report" => {
            ...
            // #3367 item 2: this report STARTED a review drive and was not
            // delivered, so the answer must not say "reported to orchestrator".
            ...
            match reg.rd_owner(&caller.group, &caller.agent_id) {
                ...
                // report `rd_owner` or the plan driver already claimed never
                // reaches it, so a driven worker's `done` is still the drive's
                // signal and never a second drive. Every refusal falls through
                // to the delivery below, unchanged — `rd_auto_start` says why
                ...
                None if status == "done"
                    ...
                    && reg.rd_auto_start(
                        &caller.group,
                        &caller.agent_id,
                        arg_str(args, "ref").unwrap_or(""),
                        &message,
                    ) =>
                {
                    // The trail the pane no longer gets: the board note, as a
                    // progress report leaves one. The drive's first notice
                    // carries the words to the orchestrator.
            ...
            if auto_started {
                return Ok("your report(done) started a review drive on that PR \
                           ...
                           orchestrator's pane: the drive's first notice carries it there. \
                           orrerix now waits for CI, briefs the reviewers the merge gate \
                           requires, and hands any findings back to this session."
                    .into());
            }
