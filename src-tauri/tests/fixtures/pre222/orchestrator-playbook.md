...
it without judgement by RECORDING it instead of typing it into your pane: a `notify_when` run or
PR-checks watch that came back green AND whose registered note named nothing to do when it did, a
planner that posted its plan and exited, a pane that exited having printed something, a cancelled
drive, and the middle chunks of a plan split `---BEGIN PLAN k/n---`. Everything else is delivered,
...
5. Only then, schedule the next item.
