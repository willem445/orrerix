...
Related: `docs/design/architecture.md` (the module map), `docs/design/engine-transport.md`
(#905 — the same cut line, on the frontend side), `docs/design/remote-engine-protocol.md`
(#888 slice B1 — the wire contract the daemon will speak),
`docs/design/groupid-and-path-roots.md` (#904).
...
`docs/design/harness-adapters.md`; the behavioural-silence bar below is the bar
...
- **A3 — the host-edge tier.** Planned in detail on #888 (plan-558): the
  ...
  - **Batch 7 — `obs`, split at its own section marker.** The first batch that
    ...
    `version: 0.0.0` while `docs/design/crash-observability.md` goes on promising
  ...
  - **Batch 9 — `subproc`, `fsatomic`: the two "host primitives" that were not
    ...
    (`docs/design/lock-liveness.md` §4.1). It is not the only durable-write
