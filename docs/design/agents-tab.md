...
> The user-facing page for the Agents tab is [`docs/features/agents-tab.md`](../features/agents-tab.md) —
...
| 4 | `attention` | the reason is urgent per `attention.ts` (`held-dialog`, `blocked`, `provider-limit`, `dialog`, `stranded`) |
...
`.sidedock`, and a third would need the argument `docs/design/side-dock.md` and
`docs/design/xterm-resize-reflow.md` describe before it may exist. Two tabs
...
(`docs/design/polled-views.md`: one read per strip). So `TabBar` gained an
...
- **No keyboard chord.** Out of scope for this slice — a new chord needs the
  `agent-cli-reference` sweep `docs/design/side-dock.md` describes, and the two
