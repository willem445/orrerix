...
- **Attention routing** — a pane earns a pulsing **needs-attention** chip when an
  ...
  - **Provider limits.** When the account behind a pane's model runs out of
    ...
    URGENT chip nothing you type in the terminal can clear (the amber gate chip
    is not terminal-clearable either — it comes down when the task moves on the
    board) — and because it
...
**Explore the plot.** The default counter is all tokens (not cache reads); the
...
done per day, median time to completion, and average tokens per pane. The
number of samples (`n`) and any unavailable values are shown with the metric.
The detail tables retain their before/after columns for a selected mark. Audit
metrics are limited to the rows still in the audit window; the chart identifies
when history may be truncated or a bucket grid was coarsened.
...
merges keep the PR body as discussion rather than history, leave it out. On a
gate that declares the condition, the gate line the orchestrator reads agrees
with the refusal: while a pass has drifted and no verification round has
...
that would be committed — so the gate line keeps saying SATISFIED, with a note
that the drift was accepted rather than a warning to re-record. Everything
else is unchanged — a `fail` still blocks, a lane that has recorded nothing
still keeps the gate shut, a pass at an older commit is still stale, and a
further body edit spends the verification and re-opens the question. Only the
driver can produce one of these, so if you record verdicts by hand the clause
...
head is stood down rather than left to finish** — one line is queued into its pane asking it to stop and
record no verdict (queued, never an interrupt: it reads it when it next takes input), and its pane is
freed once it reports. Its conversation is kept for the re-brief at the rebased head, and no review round
is charged for the reading it never got to deliver.
