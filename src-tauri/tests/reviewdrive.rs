...
///
/// Always a refusal: no integration test has the `AppHandle` `kill_agent_as`
/// needs, so a kill this guard PASSES still ends in an error — a different one,
/// which is exactly what the assertions below read.
fn orch_kill_err(
    reg: &OrchRegistry,
    group: &GroupId,
    orch: &str,
    ...
    format!("{:?}", out.expect_err("test mode has no PtyManager, so no kill can succeed"))
}
