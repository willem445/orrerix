...
does not claim any posture is a sandbox. `docs/design/orchestration.md`
...
| `driver` | select `structured` (#2850) — HOW loomux drives the block's agent: over its CLI's structured-protocol surface instead of a scraped PTY | closed enum (`DRIVER_MODES`), with the two-stage check `effort`/`context` use: refused outright outside the vocabulary, and refused on a block whose `cli:` carries no `structured_driver` in `CLI_CAPS` (today: pi only — claude's decoder exists but its spawn wiring is #84 R2, so its row refuses until then). Inert until the spawn path lands (#2850 S3b) |
| `cache_ttl_minutes` | state the prompt-cache TTL the block's agent really gets (#3407) — what the pane's cache-age chip and the orchestrator's idle-compact nudge measure against | a number, 0..=1440, refused (not clamped) above a day; it grants nothing, names no path and reaches no command line. Absent = the CLI's `CliCaps.cache_ttl_minutes` default; `0` = unknown. See [cache-age.md](cache-age.md) |
...
That is `docs/design/orchestration.md`'s Principle 3 — *guardrails in the
...
`docs/design/review-driver.md` §4.
...
`docs/design/orchestration.md` → *Honest bypass surface* says of the human merge gate
...
|---|---|---|---|---|
| `worker-std` | worker | pi | `openrouter/z-ai/glm-5.3-flash` (thinking `high`) | the DEFAULT worker: briefs the orchestrator can write literally — exact files, commands and acceptance checks. Not for work whose deliverable is evidence about itself |
...
`test/**`, `e2e/**`, `scripts/**`, `npm/**`, `docs/design/**`, `.github/workflows/**`, or
...
rule. Neither is ever renamed on the repo's behalf (`docs/design/rebrand-filesystem.md`).
