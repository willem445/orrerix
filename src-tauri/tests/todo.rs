...
fn refusal(reg: &OrchRegistry, c: &Caller, name: &str, args: Value) -> String {
    let (err, text) = call(reg, c, name, args);
...
fn mcp_a_solo_pane_gets_no_todo_tools_at_all() {
    ...
    for tool in TODO_TOOLS {
        ...
        let text =
            refusal(&reg, &cs, tool, json!({ "id": "td-1", "title": "x", "done": true }));
        assert!(
...
fn mcp_a_worker_can_add_groom_complete_and_delete_a_workspace_todo() {
    ...
    let text = refusal(&reg, &cw, "todo_get", json!({ "id": id }));
    assert_eq!(
        text,
...
fn mcp_an_id_in_another_groups_workspace_reads_as_unknown_and_is_audited() {
    ...
    for (tool, args) in [
        ...
        let text = refusal(&reg, &wb, tool, args);
        assert_eq!(
            text,
            format!("unknown todo: {id}"),
...
fn mcp_a_stale_if_rev_is_refused_and_names_both_revs() {
    ...
    let text =
        refusal(&reg, &cw, "todo_update", json!({ "id": id, "if_rev": stale, "title": "two" }));
    assert_eq!(
        text,
...
fn mcp_a_cap_refusal_comes_back_as_a_tool_error_and_is_audited() {
    ...
    let text = refusal(&reg, &cw, "todo_add", json!({ "title": over }));
    assert_eq!(
        text,
...
fn mcp_a_malformed_argument_is_refused_before_anything_is_written() {
    ...
    let text = refusal(&reg, &cw, "todo_add", json!({ "title": "x", "scope": "everywhere" }));
    ...
    let id = added["id"].as_str().unwrap().to_string();
    let text = refusal(&reg, &cw, "todo_update", json!({ "id": id, "if_rev": "1", "title": "y" }));
